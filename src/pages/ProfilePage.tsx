...
export const ProfilePage: React.FC<ProfilePageProps> = ({
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900">
      <StarsBg />
      <header className="relative z-10 border-b border-purple-500/30 bg-slate-900/50 backdrop-blur-lg">
        <div className="max-w-7xl mx-auto px-6 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div onClick={onBackToMain} className="w-10 h-10 bg-gradient-to-br from-purple-500 to-blue-500 rounded-lg flex items-center justify-center cursor-pointer hover:opacity-80 transition-opacity">
                <Database className="w-6 h-6" />
              </div>
              <h1 className="text-xl font-bold text-white">Exoplanet Hunter AI - My Profile</h1>
            </div>
            <div className="flex items-center gap-4">
              <button onClick={onBackToMain} className="px-4 py-2 bg-slate-700/50 hover:bg-slate-700 rounded-lg text-sm transition-colors">Back to Main</button>
              <button onClick={onSignOut} className="px-4 py-2 bg-slate-700/50 hover:bg-slate-700 rounded-lg text-sm transition-colors">Sign Out</button>
            </div>
          </div>
        </div>
      </header>
      ...
      <main className="relative z-10 max-w-7xl mx-auto px-6 py-8">
        {/* Profile Header Card */}
        <div className="bg-gradient-to-r from-purple-900/40 to-blue-900/40 backdrop-blur-lg rounded-2xl p-8 border border-purple-500/30 mb-6">
          <div className="flex items-start justify-between">
            <div className="flex items-center gap-6">
              <div className="w-24 h-24 bg-gradient-to-br from-purple-500 to-blue-500 rounded-2xl flex items-center justify-center text-3xl font-bold shadow-xl">{userData.photo}</div>
              <div>
                ...
                <div className="flex items-center gap-4 text-sm text-gray-400">
                  ...
                  <span className="flex items-center gap-1"><Award className="w-4 h-4 text-purple-400" />Global Rank #{userData.rank}</span>
                </div>
              </div>
            </div>
            <button onClick={() => setShowEditProfile(true)} className="px-4 py-2 bg-slate-700/50 hover:bg-slate-700 rounded-lg text-sm transition-colors flex items-center gap-2">
        ...
        <div className="grid grid-cols-12 gap-6">
          ...
          <div className="col-span-8">
            ...
            {/* History Tab */}
            {activeTab === 'history' && (
              <div className="bg-slate-800/50 backdrop-blur-lg rounded-xl border border-purple-500/20">
                <div className="p-4 border-b border-purple-500/20">
                  <div className="flex gap-3">
                    <div className="flex-1 relative">
                      <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" />
                      <input type="text" className="w-full bg-slate-700/50 border border-purple-500/20 rounded-lg pl-10 pr-4 py-2 text-sm focus:outline-none focus:border-purple-500" placeholder="Search analyses..." />
                    </div>
                    <button className="px-4 py-2 bg-slate-700/50 hover:bg-slate-700 rounded-lg text-sm flex items-center gap-2 transition-colors"><Filter className="w-4 h-4" />Filter</button>
                    <button className="px-4 py-2 bg-purple-600 hover:bg-purple-700 rounded-lg text-sm flex items-center gap-2 transition-colors"><Download className="w-4 h-4" />Export</button>
                  </div>
                </div>
                  ...
                  <table className="w-full">
                    ...
                    <tbody className="text-sm">
                      {analysisHistory.map((item, i) => (
                        <tr key={item.id} className="border-t border-purple-500/10 hover:bg-slate-700/20 transition-colors">
                          <td className="p-3">
                ...
                <div className="p-4 border-t border-purple-500/20 flex items-center justify-between text-sm">
                  <span className="text-gray-400">Showing 1-{analysisHistory.length} of {userData.totalAnalyses} analyses</span>
                  <div className="flex gap-2">
                    <button className="px-3 py-1 bg-slate-700/50 hover:bg-slate-700 rounded transition-colors">Previous</button>
                    <button className="px-3 py-1 bg-purple-600 rounded">1</button>
                    <button className="px-3 py-1 bg-slate-700/50 hover:bg-slate-700 rounded transition-colors">2</button>
                    <button className="px-3 py-1 bg-slate-700/50 hover:bg-slate-700 rounded transition-colors">3</button>
                    <button className="px-3 py-1 bg-slate-700/50 hover:bg-slate-700 rounded transition-colors">Next</button>
                  </div>
                </div>
              </div>
            )}
          ...
          <div className="col-span-4">
            {/* Achievements */}
            <div className="bg-slate-800/50 backdrop-blur-lg rounded-xl p-6 border border-purple-500/20 mb-4">
              ...
              <div className="space-y-3">
                {achievements.map((achievement, i) => (
                  <div key={i} className={`p-3 rounded-lg border transition-all ${achievement.unlocked ? 'bg-purple-900/20 border-purple-500/30 hover:bg-purple-900/30' : 'bg-slate-700/20 border-slate-600/30 opacity-50'}`}>
      ...
      {showEditProfile && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-6 bg-black/70 backdrop-blur-sm">
          <div className="bg-slate-800/95 backdrop-blur-xl rounded-2xl p-8 border border-purple-500/30 max-w-md w-full">
            ...
            <div className="space-y-4">
              <div>
                ...
                <div className="flex items-center gap-4">
                  <div className="w-20 h-20 bg-gradient-to-br from-purple-500 to-blue-500 rounded-full flex items-center justify-center text-2xl font-bold">{userData.photo}</div>
              ...
              <div>
                ...
                <input type="text" defaultValue={userData.name} className="w-full bg-slate-700/50 border border-purple-500/20 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-purple-500" />
              </div>
              <div>
                <label className="text-sm text-gray-300 mb-2 block">Email Address</label>
                <input type="email" defaultValue={userData.email} disabled className="w-full bg-slate-700/30 border border-purple-500/20 rounded-lg px-4 py-3 text-gray-400 cursor-not-allowed" />
                <p className="text-xs text-gray-500 mt-1">Email cannot be changed</p>
              </div>
              <div>
                <label className="text-sm text-gray-300 mb-2 block">New Password (Optional)</label>
                <input type="password" placeholder="Leave blank to keep current password" className="w-full bg-slate-700/50 border border-purple-500/20 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-purple-500" />
              </div>
              <div className="flex gap-3 pt-4">
                ...
                <button onClick={() => setShowEditProfile(false)} className="flex-1 bg-gradient-to-r from-purple-600 to-blue-600 hover:from-purple-700 hover:to-blue-700 rounded-lg py-3 font-semibold transition-all flex items-center justify-center gap-2">
